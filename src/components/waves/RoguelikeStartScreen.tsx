...
export type RunType = "standard" | "beachBonanza" | "bossQuickRun" | "bossHellRun" | "slayTheWaves";
...
const RUN_TYPES: { type: RunType; name: string; description: string }[] = [
  { type: "standard", name: "Standard Run", description: "Progressive difficulty with random boss beaches every 5 levels" },
...
const RoguelikeStartScreen = ({ onStart, onContinue, hasSavedRun, savedRunType }: RoguelikeStartScreenProps) => {
  ...
  const [selectedRunType, setSelectedRunType] = useState<RunType>(() => {
    const saved = localStorage.getItem("waveChaser_runType");
  ...
  useEffect(() => {
    localStorage.setItem("waveChaser_runType", selectedRunType);
  }, [selectedRunType]);
